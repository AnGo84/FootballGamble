...
public class CompetitionServiceImpl implements CommonService<CompetitionEntity> {

	private static final String ENTITY_PATH = "/competitions";

	public static final Logger logger = LoggerFactory.getLogger(CompetitionServiceImpl.class);
	...
	public List<CompetitionEntity> findAll()
		...
		logger.info("Get All Competitions List");
		List<CompetitionEntity> competitionsList = new ArrayList<>();

		RestTemplate restTemplate = new RestTemplate();
		restTemplate.setErrorHandler(new RestTemplateResponseErrorHandler());
		// Example: https://www.baeldung.com/spring-rest-template-list
		ResponseEntity<List<CompetitionEntity>> response = restTemplate.exchange(apiPath + ENTITY_PATH, HttpMethod.GET,
				httpHeaders.getHttpAuthEntity(), new ParameterizedTypeReference<List<CompetitionEntity>>() {
				});
		competitionsList = response.getBody();
	...
	public CompetitionEntity findById(long id)
			throws AuthorisationException, NotFoundException, DataConflictException, RestAPIServerException {
		logger.info("Get Competition by id: " + id);
		RestTemplate restTemplate = new RestTemplate();
		restTemplate.setErrorHandler(new RestTemplateResponseErrorHandler());
		ResponseEntity<CompetitionEntity> response = restTemplate.exchange(apiPath + ENTITY_PATH + "/" + id,
				HttpMethod.GET, httpHeaders.getHttpAuthEntity(), CompetitionEntity.class);
		CompetitionEntity competition = response.getBody();
		logger.info("Getted CompetitionEntity from API: " + competition);
		return competition;
	}
	...
	public void updateFromAPI(CompetitionEntity object)
			throws AuthorisationException, NotFoundException, DataConflictException, RestAPIServerException {
		logger.info("Update Competition from API: " + object);
		RestTemplate restTemplate = new RestTemplate();
		restTemplate.setErrorHandler(new RestTemplateResponseErrorHandler());
		ResponseEntity<CompetitionEntity> response = restTemplate.exchange(
				apiPath + ENTITY_PATH + "/updatefromapi/" + object.getId(), HttpMethod.GET,
				httpHeaders.getHttpAuthEntity(), CompetitionEntity.class);
		CompetitionEntity competition = response.getBody();
	...
	@Override
	public boolean isExist(CompetitionEntity competition)
			throws AuthorisationException, NotFoundException, DataConflictException, RestAPIServerException {
